...
div.container
  ...
  uni-popup(ref="searchFormPopupOb" type="bottom" :mask-click="false")
    div.search_form
      template(v-for="(item, index) in props.columns" :key="index")
        ...
        template(v-if="item.formType === 'select'" )
          uni-section(:title="item.title" titleFontSize="36rpx")
          uni-data-checkbox.select_vip(v-model="searchForm[item.index]" :localdata="item.data" selectedColor="#8c40d9" selectedTextColor="#8c40d9")
